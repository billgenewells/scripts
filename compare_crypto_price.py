...
def cryptocompare(btcpercent, ethpercent):
    ...
    btcmess = 'Bitcoin is currently $%s, %s%% difference' % (btcnowrate, round(btcdifference*100, 2))
    ethmess = 'Ethereum is currently $%s, %s%% difference' % (ethnowrate, round(ethdifference*100, 2))

    print btcmess
    print ethmess

    if btcdiff/btcyesrate >= btcpercent:
        SNSCLIENT.publish(
    ...
    if ethdiff/ethyesrate >= ethpercent:
